...
describe("Market", function() {
  ...
  it("addLiquidityETH burns for Uniswap router", async () => {
    ...
    await addLiquidityETH({ router, wallet, token: latte, tokenAmount, ethAmount })
    expect(await latte.balanceOf(wallet.address)).eq(expandDecimals(1000 - 200 - 6, 18))

    await addLiquidityETH({ router: market, wallet, token: latte, tokenAmount, ethAmount })
    expect(await latte.balanceOf(wallet.address)).eq(expandDecimals(1000 - 200 - 200 - 6, 18))
  })
  ...
  it("sellTokens", async () => {
    ...
    await sellTokens({ router, wallet, weth, token: latte, tokenAmount: expandDecimals(100, 18) }) // burn 3
    expect(await latte.balanceOf(wallet.address)).eq(expandDecimals(500 - 100 - 3, 18))
  })
